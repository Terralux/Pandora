...
public class Toolbox : Singleton<Toolbox>
{
	...
	public List<BaseEvent> registeredEvents = new List<BaseEvent>();
	...
	static public T RegisterEvent<T>(BaseEvent baseEvent) where T : BaseEvent {
		
		Instance.registeredEvents.Add(baseEvent);

		for (int j = Instance.registeredEvents.Count - 1; j > 0; j--) {
			for (int i = 0; i < j; i++) {
				if (Instance.registeredEvents [i].CompareTo (Instance.registeredEvents [i + 1]) > 0) {
					BaseEvent be = Instance.registeredEvents [i];
					Instance.registeredEvents [i] = Instance.registeredEvents [i + 1];
					Instance.registeredEvents [i + 1] = be;
				}
			}
		}

		for (int i = 0; i < Instance.registeredEvents.Count; i++) {
			Debug.Log (Instance.registeredEvents [i].eventTriggerIndex);
		}
